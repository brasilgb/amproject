...
const EditCustomer = (tenants: any) => {

  const { data, setData, patch, progress, processing, errors } = useForm({
    codigo: tenants.tenants.id,
    descricao: tenants.tenants.descricao,
    endereco: tenants.tenants.endereco,
    numero: tenants.tenants.numero,
    cep: tenants.tenants.cep,
    municipio: tenants.tenants.municipio,
    uf: tenants.tenants.uf,
    bairro: tenants.tenants.bairro,
    cnpj: tenants.tenants.cnpj,
    inscricao: tenants.tenants.inscricao,
    telefone: tenants.tenants.telefone,
    whatsapp: tenants.tenants.whatsapp,
    obs: tenants.tenants.obs,
  });

  function handleSubmit(e: any) {
    e.preventDefault();
    patch(route("customers.update", tenants.tenants.id));
  }
